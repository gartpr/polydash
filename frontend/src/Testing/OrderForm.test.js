...
test('renders the empty form correctly', () => {
  ...
  const paymentInfoInput = screen.getByPlaceholderText(
...
test('allows input in each box', () => {
  ...
  fireEvent.change(commentInfoInput, {
    target: { value: 'Special instructions' },
  });
...
test('allows input in each box and submits the form', async () => {
  ...
  setTimeout(() => {
    expect(CartProvider.useCart.removeFromCart).toHaveBeenCalledWith(
      'mockItemId1',
    );
  }, 0);
});
...
test('handles successful order placement', async () => {
  ...
  setTimeout(() => {
    ...
    expect(global.console.log).toHaveBeenCalledWith(
      'Order document added successfully:',
      'mockOrderId',
    );
