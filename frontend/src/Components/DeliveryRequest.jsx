    ...
    const ButtonSelection = (orderStatus) => {
        switch (orderStatus.orderStatus) {
            ...
            case "Delivered":
                return <div></div>
            default:
                return <Button colorScheme="green" onClick={() => acceptDelivery(order.id)}>
                            Accept
                        </Button>
        }
    }
            ...
            <h2>
                <AccordionButton>
                    <Box flex="1" textAlign="left">
                        <Text mt={2} fontWeight="bold" color="#B38F4F">Pickup Location:</Text>
                        <Text mt={2} fontWeight="bold" mb={10}>{restaurant.name} - {restaurant.adress}</Text>
                        <Text mt={2} fontWeight="bold" color="#B38F4F">Destination:</Text>
                        <Text mt={2} fontWeight="bold">{order.address}</Text>
                    </Box>
                    <AccordionIcon />
                </AccordionButton>
            </h2>
            <AccordionPanel pb={4}>
                <Text fontWeight="bold" mt={10} color="#B38F4F">Customer Name:</Text>
                <Text mb={5}>{order.customerName}</Text>
                <Text fontWeight="bold" color="#B38F4F">Resteraunt:</Text>
                <Text mb={5}>{restaurant.name}</Text>
                <Text fontWeight="bold" color="#B38F4F">Delivery Fee:</Text>
